...
void __generatePaths_kernel_cpu(__float2* numeraires, int timepoints,
    float* drifts, float* volatilities, float* rngNrmVar,
    float* spot_rates,
    float* simulated_rates, float* simulated_rates0, float* accum_rates,
    const int pathN, int path,
    float dtau = 0.5, float dt = 0.01)
{
    ...
    for (int s = 0; s < stride; s++) {
        // Normal variates
        float phi0 = rngNrmVar[(path + s) * 3];
        float phi1 = rngNrmVar[(path + s) * 3 + 1];
        float phi2 = rngNrmVar[(path + s) * 3 + 2];
 ...
 */
void __calculateExposure_kernel(float* exposure, __float2* numeraires, const float notional, const float K, float* accrual, int simN, double dtau = 0.5f)
{
    ...
    for (int t = 0; t < _TIMEPOINTS; t++) {
        ...
        float libor = m * (exp(forward_rate / m) - 1.0f);
